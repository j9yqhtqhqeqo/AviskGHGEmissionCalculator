...
def test_csv_file_access():
    ...
    # Test the file path
    csv_file_path = os.path.join(os.path.dirname(__file__), '..', 'Data', 'Co2TestDataFreightDistance.csv')
        ...
        if os.path.exists(file_path):
            try:
                ...
                if 'Co2TestDataFreightDistance.csv' in content:
                    print(f"✅ {test_file}: Updated to use Co2TestDataFreightDistance.csv")
                elif 'Co2TestData.csv' in content:
                    print(f"⚠️  {test_file}: Still references old Co2TestData.csv")
                else:
